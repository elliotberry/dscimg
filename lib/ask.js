...
export const ask = async (blob, filename) => {
  ...
  const requestOptions = {
    ...
    if (text.success === false) {
        console.error(text)
        const textError = text.error.map((error) => `${error.code}: ${error.message}`).join("\n")
    ...
    return {
        filename: slugged.removePrefix(),
        time: `${Math.floor(endTime - startTime)}ms`,
        prompt,
        result: text.result,
    }
}
